...
export const useScrollFadeIn = (direction, duration, delay, changePercent) => {
  ...
  const hanldeDirection = (name) => {
    switch (name) {
          case "up": return `translate3d(0, ${changePercent}%, 0)`
          case "down": return `tranlate3d(0, -${changePercent}%, 0)`
          case "left": return `translate3d(${changePercent}%, 0, 0)`
          case "right": return `translate3d(-${changePercent}%, 0, 0)`
          derault: return;
      }
  }
